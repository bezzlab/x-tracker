...
public class xPeaks {
     ...
     */
    public xPeaks() {
        correspondences = new ArrayList<xCorrespondences>();
    }
    ...
    public xCorrespondences getElemAt(int index) {
        if (index < correspondences.size()) {
            return correspondences.get(index);
        } else {
            return null;
        }
    }
     ...
     * <p>If the same couple proteinId, peptideSeq is present more than once, just a xCorrespondenceData element will be created with more than one xLcMsCorr or xLcMsMsCorr elements.
     *
     ...
     * @see xCorrespondenceData
     */
    private List<xCorrespondences> correspondences;
}
