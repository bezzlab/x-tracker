...
public class xQuant {
     ...
     */
    public xQuant() {
        quantificationData = new ArrayList<xQuantData>();
    }
    ...
    public xQuantData getElementAtIndex(int i) {
        if (i < this.quantificationData.size()) {
            return quantificationData.get(i);
        }
        return null;
    }
    ...
    public boolean isValid() {
        ...
        for (i = 0; i < size; i++) {
            ret = (ret && quantificationData.get(i).isValid());
        }
        return (ret && notEmpty);
    }
    /**
     ...
     * @see xQuantData
     */
    private List<xQuantData> quantificationData;
}
