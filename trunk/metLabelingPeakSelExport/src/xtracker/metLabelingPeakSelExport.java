...
public class metLabelingPeakSelExport implements peakSelPlugin
{
    /**
    ...
    public xPeaks start(xLoad input, String paramFile)
    {
        ...
        String outMatFile="./outMatlab.m";
        try{
        outFile = new FileWriter(outMatFile);
                    ...
                    if(howMany==outOf){
                        ...
                        //compute their peak correspondences.
                         ...
                         if(lightSize>0 && heavySize>0){
                                ...
                                outF.println("%%%%%");
                                outF.println("%% Light version of " + pepSeq + ":");
                                ...
                                for(int li=0;li<toConsider; li++){
                                    ...
                                    outF.println("%% Light mz:" + myXIdntDt.getParentMass() + " (at rt:" + myXIdntDt.getRetTime() +") charge:+"+  myXIdntDt.getCharge());
                                    outF.println("%%%%%");
                                    //Ok everything is ready to compute the peak-correspondences for hte light version of the peptide
                                    computePeakCorrespondences("Light",pepSeq,pepId, myXIdntDt.getParentMass(), myXIdntDt.getCharge(), myXIdntDt.getRetTime(), myXCorrData, myLoadData,myXIdntDt.getAllModifications(), outF);
                                    

                                //Now it's time to compute peak correspondences of the heavy version.
                                System.out.println("Heavy version of " + pepSeq + ":");
                                outF.println("%%%%%");
                                outF.println("%% Heavy version of " + pepSeq + ":");
                                for(int h=0;h<toConsider; h++){
                                    //String indexes[] = new String[2];
                                    indexes=heavyPeps.elementAt(h).split(":");
                                    myDoubIdentData= myIdent.elementAt(Integer.valueOf(indexes[0]).intValue());
                                    ...
                                    outF.println("%% Heavy mz:" + myXIdntDt.getParentMass() + " (at rt:" + myXIdntDt.getRetTime() +") charge:+"+  myXIdntDt.getCharge());
                                    outF.println("%%%%%");
                                    //Ok everything is ready to compute the peak-correspondences for hte heavy version of the peptide
                                    computePeakCorrespondences("Heavy",pepSeq,pepId, myXIdntDt.getParentMass(), myXIdntDt.getCharge(), myXIdntDt.getRetTime(), myXCorrData, myLoadData, myXIdntDt.getAllModifications(), outF );
                                ...
                                if(lightSize>0){
                                    ...
                                    outF.println("%%%%%");
                                    outF.println("%% JUST Light version of " + pepSeq + ":");
                                    for(int li=0;li<lightPeps.size(); li++){
                                        ...
                                        xIdentData myXIdntDt=myDoubIdentData.getLcMsMsElemAt(Integer.valueOf(indexes[1]).intValue());
                                        System.out.println("- Light mz:" + myXIdntDt.getParentMass() + " (at rt:" + + myXIdntDt.getRetTime() +") charge:+"+  myXIdntDt.getCharge());
                                        outF.println("%% JUST Light mz:" + myXIdntDt.getParentMass() + " (at rt:" + myXIdntDt.getRetTime() +") charge:+"+  myXIdntDt.getCharge());
                                        outF.println("%%%%%");
                                        ...
                                        outF.println("%%%%%");
                                        outF.println("%% COMPUTED Heavy mz:" + (myXIdntDt.getParentMass()+shift/myXIdntDt.getCharge()) + " (at rt:" + + myXIdntDt.getRetTime() +") charge:+"+  myXIdntDt.getCharge());
                                        outF.println("%%%%%");

                                        computePeakCorrespondences("Heavy",pepSeq,pepId, myXIdntDt.getParentMass()+(shift/myXIdntDt.getCharge()), myXIdntDt.getCharge(), myXIdntDt.getRetTime(), myXCorrData, myLoadData,myXIdntDt.getAllModifications(), outF );
                                    ...
                                    outF.println("%%%%%");
                                    outF.println("%% JUST Heavy version of " + pepSeq + ":");
                                    ...
                                    for(int h=0;h<heavyPeps.size(); h++){
                                        ...
                                        outF.println("%% JUST Heavy mz:" + myXIdntDt.getParentMass() + " (at rt:" + myXIdntDt.getRetTime() +") charge:+"+  myXIdntDt.getCharge());
                                        outF.println("%%%%%");

                                         Vector<Float> rtH = new Vector<Float>();
                                         Vector<Float> intH = new Vector<Float>();
                                         Vector<xModification> mods = myXIdntDt.getAllModifications();
                                        ...
                                        outF.println("%%%%%");
                                        outF.println("%% COMPUTED Light mz:" + (myXIdntDt.getParentMass()-(shift/myXIdntDt.getCharge())) + " (at rt:" + myXIdntDt.getRetTime() +") charge:+"+  myXIdntDt.getCharge());
                                        outF.println("%%%%%");
                                        computePeakCorrespondences("Light",pepSeq,pepId, myXIdntDt.getParentMass()-(shift/myXIdntDt.getCharge()), myXIdntDt.getCharge(), myXIdntDt.getRetTime(), myXCorrData, myLoadData,myXIdntDt.getAllModifications(), outF );
    ...
    public void computePeakCorrespondences(String type,String pepSeq,String pepId, float pMz, int pepCharge, float rt, xCorrespondenceData peaksCorr, xLoadData data,Vector<xModification> mods,PrintWriter outFile ){
...
*/            
 ...
 //       }
  ...
  for(int j=indLow;j<=indHigh; j++){
                  ...
                  outFile.print(XICrt.elementAt(j) + " " + XICint.elementAt(j) +";");
                for(int kk=0;kk<mods.size();kk++){
                    xModification myMod=mods.elementAt(kk);
                    String modName=myMod.getName();
                    int position=myMod.getPosition();
...
*/
   ...
   public float[] getStartEndRT(Vector<Float> rtV, Vector<Float> intV, float retTime){
        ...
        //List<Float> tmpRT= new Vector<Float>();
        ...
        int lowerInd=Collections.binarySearch(rtV, retTime-this.retTimeWindow);
        if(lowerInd<0){
            lowerInd=0-lowerInd-2;
        }
        int upperInd=Collections.binarySearch(rtV, retTime+this.retTimeWindow);
        if(upperInd<0){
...
/*                for(int j=0; j<myLcMsData.getSize();j++){
    ...
    private final static String name = "Metabolic Labeling (experimental)";
