...
/**
 * Class that contains all the methods used to compute the customised APEX quantification, where the user
 * can provide a file with the Oi value of each identified protein.
 * These methods are called by other methods in this file, or by the quantAPEXcustom.java class.
...
public class methods {
    ...
    public static double massPeptide(String peptide) {
        ...
        char[] AAs = {'A', 'R', 'N', 'D', 'C', 'G', 'E', 'Q', 'H', 'I', 'L', 'K', 'M', 'F', 'P', 'S', 'T', 'W', 'Y', 'V'};
        double[] mono_mass = {71.03711, 156.10111, 114.04293, 115.02694, 103.00919, 57.02146, 129.04259, 128.05858, 137.08406, 113.08406, 113.08406, 128.09496, 131.04049, 147.06841, 97.05276, 87.03203, 101.04768, 186.07931, 163.06333, 99.06841};
    ...
    /**
     * Methods that reads a csv file containing the Oi values of all the proteins of a list, and put for each protein
     * its Oi value.
     * @param filename The path to the file containing the Oi values
     * @param listOfProt The list of proteins that need an Oi value
     */
    public static void readOi(String filename, APEXProteinList listOfProt){

        // First read the csv file and store the values in an Hash Map, then read this map to
        // associate each protein with its Oi value.
        ...
        try{
            ...
            if(!(filename.contains(".csv"))){
               System.out.println("WARNING: the file with the Oi values is not a csv file");
            ...
            if(!(l.length==2)){
                System.out.println("WARNING: the file with the Oi values is not at a proper format");
                System.out.println("impossible to continue");
                System.exit(1);
            }
            ...
            while ((line = br.readLine()) != null) {
                ...
                values.put(SeparatedLine[0], SeparatedLine[1]);
            ...
            System.err.println("WARNING: An error occured reading the Oi file "+filename);
            System.err.println("impossible to continue quantification");
            System.exit(1);
        }

        // ++++++++++++++++++++++++++++++++++++++++++
        // associate each protein with its value
        // ++++++++++++++++++++++++++++++++++++++++++

        for(int i=0; i<listOfProt.size(); i++){

            APEXProtein CurrentProt=listOfProt.getProteinAt(i);
           

            if(values.containsKey(CurrentProt.getId())){
                String obj=(String)values.get(CurrentProt.getId());
               
               CurrentProt.setOi(Double.parseDouble(obj));
            }
            else{
                System.out.println("WARNING: A protein does not have an Oi value in the Oi file");
                System.out.println("impossible to continue quantification");
                System.exit(1);
            }
        }
